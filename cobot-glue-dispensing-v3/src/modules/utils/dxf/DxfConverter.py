...
class DxfConverter:
    ...
    def __init__(self, dxf_file, dpi=146, target_size_mm=(748, 498), target_size_px=(1280, 720),
                 mode='png'):
        """
    ...
    def scale_contours(self, contours):
        ...
        print("Scale X", self.scaleX)
        print("Scale Y", self.scaleY)
    ...
    def process_dxf(self):
        ...
        if self.first_bbox:
            ...
            self.draw_rectangle(center_x, center_y, self.target_size_mm[0], self.target_size_mm[1])
    ...
    def handle_contours(self):
        ...
        print("Scaled contours: ", len(scaledContours))

        # Ensure contours are in the correct format
        formatted_contours = [np.array(contour, dtype=np.int32) for contour in scaledContours]

        # filter contours by area
        formatted_contours = [cnt for cnt in formatted_contours if cv2.contourArea(cnt) < 800000]

        formatted_contours = [contour.tolist() for contour in formatted_contours]
        self.contours_to_json_file(formatted_contours, 'contours.json')
        return 'contours.json', formatted_contours
