...
class RobotCalibrationPipeline:
    def __init__(self, config:RobotCalibrationConfig,
        ...
        # --- STATES ---
        if adaptive_movement_config is not None:
            self.adaptive_movement_config = adaptive_movement_config
            self.alignment_threshold_mm = self.adaptive_movement_config.target_error_mm
            self.adaptive_movement = True  # Use larger moves when error is large
        else:
            self.adaptive_movement = False
            self.adaptive_movement_config = None
        ...
        self.logger_context = LoggerContext(ENABLE_LOGGING,robot_calibration_logger,self.broadcast_events)
        ...
        self.robot_service = config.robot_service
    ...
    def compute_ppm_from_corners(self, corners_refined):
        ...
        log_debug_message(self.logger_context,message=f"PPM calculation debug: avg_square_px={avg_square_px:.3f}, square_size_mm={self.square_size_mm:.2f}, ppm={ppm:.3f}")
    ...
    def find_chessboard_and_compute_ppm(self, frame):
        ...
        if ret:
            ...
            log_debug_message(self.logger_context,f"Bottom-left chessboard corner (px): {self.bottom_left_chessboard_corner_px}")
            ...
            log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.DEBUG,
    ...
    def find_required_aruco_markers(self, frame):
        ...
        if arucoIds is not None:
            log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.INFO, f"Detected {len(arucoIds)} ArUco markers",
                           broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
            log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.DEBUG, f"Marker IDs: {arucoIds.flatten()}",
                           broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
            ...
            log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.DEBUG, f"Currently have: {self.detected_ids}",
                           broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
            log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.DEBUG,
            ...
            if all_found:
                log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.INFO, "🎯 All required ArUco markers found!",
                               broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
    ...
    def show_live_feed(self, frame, current_error_mm=None, window_name="Calibration Live Feed", draw_overlay=True,broadcast_image=False):
        ...
        if key == ord('q'):
            log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.INFO, "Live feed stopped by user (q pressed)",
                           broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
            return True  # Signal to exit
        elif key == ord('s'):
            # Save current frame
            cv2.imwrite(f"live_capture_{time.time():.0f}.png", display_frame)
            log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.INFO, "Live frame saved",
                           broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
        elif key == ord('p'):
            # Pause/resume
    ...
    def test_calibration(self, homography_matrix, camera_points, robot_points, save_json_path=None):
        ...
        for i, (cam_pt, transformed, robot_pt) in enumerate(zip(cam_pts, transformed_pts_flat, rob_pts)):
            ...
            log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.DEBUG, f"Point {i + 1}:",
                           broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
            log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.DEBUG, f"  Camera point:      {cam_pt}",
                           broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
            log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.DEBUG, f"  Transformed point: {transformed}",
                           broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
            log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.DEBUG, f"  Robot point:       {robot_pt}",
                           broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
            log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.DEBUG, f"  Error (mm):        {error:.3f}",
                           broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
    ...
    def log_timing_summary(self):
        ...
        for state_name, percentage in sorted_bottlenecks:
            if percentage > 5:  # Only show states that take more than 5%
                log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.WARNING,
    ...
    def run(self):
        ...
        while True:
            ...
            if self.current_state == self.states["INITIALIZING"]:
                ...
                while chessboard_frame is None:
                    log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.DEBUG, "Capturing chessboard frame...",
                                   broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
                ...
                for marker_id, top_left_corner_mm in self.marker_top_left_corners_mm.items():
                    log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.DEBUG, f"ID {marker_id}: {top_left_corner_mm}",
                                   broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
                ...
                log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.INFO,
                               ...
                               broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
                result = self.move_to_position(new_position,blocking=True)


                if result != 0:
                    # move to the center of the chessboard if the move failed
                    log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.ERROR,
                                   f"Movement to marker {marker_id} failed, returning to calibration position",
                                   broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
                    # self.robot_service.moveToCalibrationPosition()

                    if len(self.robot_positions_for_calibration) != 0:
                        result = self.move_to_position(self.robot_positions_for_calibration[0],blocking=False)
                ...
                log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.DEBUG, "Computing Error:",
                               broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
                log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.DEBUG,
                ...
                if current_error_mm <= self.alignment_threshold_mm:
                    ...
                    log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.INFO,
                                   ...
                                   broadcast_to_ui=self.broadcast_events,
                ...
                result = self.move_to_position(iterative_position,blocking=True)
        ...
        log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.INFO, "Calibration process finished.",
                       broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
        log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.INFO, "Markers and recorded robot positions:",
                       broadcast_to_ui=self.broadcast_events, topic=self.BROADCAST_TOPIC)
        ...
        log_if_enabled(ENABLE_LOGGING, robot_calibration_logger, LoggingLevel.INFO, "Camera points for homography:",
...
if __name__ == "__main__":
    ...
    pipeline = RobotCalibrationPipeline(required_ids=required_ids,
                                        debug=True,
